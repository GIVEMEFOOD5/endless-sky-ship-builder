...
/* ── Wrapper keeps row + panel together ──────────────────────────── */
.${CLS.wrapper},
.${CLS.pickerWrapper} {
    display: flex;
...
/* Remove the default bottom margin from the row inside a wrapper */
.${CLS.wrapper} .outfit-item,
.${CLS.pickerWrapper} .sb-picker-row {
    margin-bottom: 0;
}
...
.${CLS.toggle}[aria-expanded="true"] {
    background: rgba(59,130,246,0.20);
    border-color: var(--c-border-hi);
    color: var(--c-accent-text);
}

/* Picker toggle sits at the end, pushed right */
.oe-toggle--picker {
    margin-left: 8px;
}
...
.${CLS.panel}::-webkit-scrollbar { width: 4px; }
.${CLS.panel}::-webkit-scrollbar-track { background: transparent; }
.${CLS.panel}::-webkit-scrollbar-thumb { background: rgba(59,130,246,0.35); border-radius: 2px; }
...
.${CLS.groupTitle}:first-child { margin-top: 0; }
...
/* ── Narrow screen: single column stat grid ───────────────────────── */
@media (max-width: 600px) {
    .${CLS.statGrid} { grid-template-columns: 1fr 1fr; }
    .${CLS.imgWrap}  { width: 56px; height: 56px; }
}
`;
