...
async function createCard(item) {
    ...
    try {
        thumbnailImage = await window.fetchSprite(item.thumbnail, null);
        spriteImage    = await window.fetchSprite(item.sprite, null);
    } catch (e) {
    ...
    img.src = spriteImage || thumbnailImage || '';
...
window.switchModalTab = switchModalTab;        if (!indexResponse.ok) throw new Error('Could not find data/index.json in repository');
        const dataIndex = await indexResponse.json();
